...
export function KeywordsAutocomplete({ 
  value = [], 
  onChange, 
  placeholder = "Search keywords & moods...",
  className 
}: KeywordsAutocompleteProps) {
  ...
  // Query for searching keywords with debounced query
  ...
  // Effect to update selected keywords when value changes externally
  useEffect(() => {
    if (value.length === 0) {
      setSelectedKeywords([]);
    }
      ...
      <Popover open={open} onOpenChange={setOpen}>
        <PopoverTrigger asChild>
          <Button
            ...
            data-testid="keywords-search-trigger"
          >
            {placeholder}
        ...
        <PopoverContent className="w-full p-0" align="start">
          <Command>
            <CommandInput
              ...
              data-testid="keywords-search-input"
            />
            <CommandList>
