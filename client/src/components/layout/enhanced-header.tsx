      ...
      <header 
        className={cn(
          ...
          isScrolled 
            ...
            : "bg-background/95 backdrop-blur-2xl border-b border-border/25"
        )}
        data-testid="header"
      >
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div className="flex items-center space-x-8">
              <Link href="/" data-testid="link-home">
                <div className="flex items-center space-x-3 cursor-pointer group">
                  <div className={cn(
                    "relative bg-gradient-to-br from-primary to-secondary rounded-xl flex items-center justify-center shadow-lg transition-all duration-300",
                    isScrolled ? "w-10 h-10" : "w-12 h-12"
                  )}>
                    <Film className={cn("text-white transition-all duration-300", isScrolled ? "w-5 h-5" : "w-6 h-6")} />
                  </div>
                  <div className="flex flex-col">
                    <span className={cn(
                      "font-display font-black bg-gradient-to-r from-primary to-secondary bg-clip-text text-transparent transition-all duration-300",
              ...
              <nav className="hidden md:flex items-center space-x-2">
                {navItems.map((item) => (
                  <Link key={item.href} href={item.href} data-testid={`link-${item.label.toLowerCase()}`}>
                    <span
                      className={cn(
                        "px-4 rounded-xl cursor-pointer transition-all duration-300 font-medium text-sm",
                  ...
                  <DropdownMenu>
                    <DropdownMenuTrigger asChild>
                      <Button variant="ghost" className="flex items-center space-x-2" data-testid="button-user-menu">
                        <Avatar className={cn("transition-all duration-300", isScrolled ? "w-7 h-7" : "w-8 h-8")}>
                        ...
                        {!isScrolled && <span className="text-sm font-medium">{user?.firstName || user?.displayName || 'User'}</span>}
          ...
          {isMobileMenuOpen && (
            <div className="md:hidden border-t border-border/30 py-6" data-testid="mobile-menu">
              <nav className="flex flex-col space-y-3">
                {navItems.map((item) => (
                  <Link key={item.href} href={item.href} data-testid={`mobile-link-${item.label.toLowerCase()}`}>
                    <span
                      className={cn(
                        "block px-5 py-4 rounded-xl cursor-pointer transition-all duration-300",
                        location === item.href
